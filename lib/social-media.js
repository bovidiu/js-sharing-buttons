(function () {
  ...
  SocialMedia.config = {
    ...
    getSpecificIcons: function(icon){
      var iconList = SocialMedia.icons.list();
      for(var icn in icon){
        if (iconList.iconList[icn]){
          console.log(iconList[icn]);
          /*var path    = SocialMedia.config.setupData();
           var url     = iconList[icn].url + path.pageURL;
          SocialMedia.config.makeAnchor(iconList[icn].fawesome.standard, '', url); */
        }
      }
  ...
  for(var i = 0; i < elem.length; i++){
    if(elem[i].hasAttribute('data-sm-icon')){
      ...
      SocialMedia.config.getSpecificIcons(iconType);
